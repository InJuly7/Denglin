...
YOLOV8::YOLOV8(const utils::InitParameter& param) {
    ...
    CHECK(cudaMalloc(&m_input_dst_device, 3 * param.dst_h * param.dst_w * sizeof(float)));
...
YOLOV8::~YOLOV8() {
    ...
    free(m_output_objects_host);

}

bool YOLOV8::init(std::vector<unsigned char>& slz, const int &length)
{
    if (slz.empty()) {
        return false;
    }
    ...
    m_output_objects_host = (float*)malloc(m_output_area * sizeof(float));
...
void YOLOV8::copy(const cv::Mat& img)
{
// #if 0 
...
{
    cudaWarpAffine(m_input_src_device, m_param.src_w, m_param.src_h, m_input_dst_device, m_param.dst_w,  m_param.dst_h, m_dst2src, 
                    paddingValue, alpha, beta);
    // utils::DumpGPUMemoryToFile(m_input_dst_device, 3 * m_param.src_h * m_param.src_w * sizeof(float), "m_input_dst_device.bin");
}
...
std::vector<utils::Box> YOLOV8::getObjectss() const
{

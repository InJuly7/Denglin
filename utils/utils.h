...
namespace utils 
{
    ...
    bool setInputStream(const utils::InputStream& source, const std::string& imagePath, const std::string& videoPath, 
    ...
    void show(const std::vector<utils::Box>& objectss, const std::vector<std::string>& classNames, const int& cvDelayTime, 
					cv::Mat& img);

    void save(const std::vector<Box>& objectss, const std::vector<std::string>& classNames, const std::string& savePath, cv::Mat& img, 
                const int& batchi);
